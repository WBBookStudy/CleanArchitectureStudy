...
#### 고통의 구역
(0,0) 주변 구역에 위치한 컴포는트는 매우 안정적이며 구체적이다. 이 상태는 추상적이지 않으므로 확장할 수 없고, 안정적이므로 변경하기도 어렵다.  
사실 DB스키마가 이곳에 위치하며, 구체적인 유틸리티 라이브러리도 이곳에 위치한다. 비록 이러한 라이브러리는 I지표가 1일지라도 실제로는 거의 변동성이 없다. ex) String  
변동성이 없는 컴포넌트는 (0,0)에 위치했더라도 해롭지 않다.  
하지만 그렇지 않다면..?

#### 쓸모없는 구역
(1,1)에 위치한 컴포넌트는 최고로 추상적이지만, 누구도 그 컴포넌트에 의존하지 않는다. 이러한 컴포넌트는 쓸모가 없다.  
이 영역에 존재하는 소프트웨어 인티티는 폐기물이다. 누구도 구현하지 않은 추상클래스인 경우가 많다. 

### 배제구역 벗어나기
따라서 변동성이 큰 컴포넌트 대부분은 두 배제 구역으로부터 멀리 떨어뜨려야 한다. 각 배제 구역으로부터 멀리 떨어진 점의 궤적은 (1,0)과 (0,1)을 잇는 선분이다. (Main Sequence)  
주계열에 위치한 컴포넌트는 자신의 안정성에 비해 너무 추상적이지도 않고, 추상화 정도에 비해 너무 불안정하지도 않다. 컴포넌트가 추상화된 수준에 어울릴 정도로만 다른 컴포넌트가 의존하며, 구체화된 수준에 어울릴 정도로만 다른 컴포넌트에 의존한다.  
